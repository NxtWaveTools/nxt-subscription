...
export async function createSubscription(
  ...
  try {
    ...
    // Auto-create first payment cycle with auto-POC approval
    try {
      ...
      const { error: cycleError } = await supabase
        ...
        .insert({
          ...
          // First cycle is auto-approved by POC (since they're requesting the subscription)
          poc_approval_status: 'APPROVED',
          poc_approved_by: currentUser.id,
          poc_approved_at: new Date().toISOString(),
          cycle_status: 'PENDING_PAYMENT', // Ready for finance to record payment
          payment_status: 'IN_PROGRESS',
          accounting_status: 'PENDING',
        })
