...
export function PaymentCycleSection({
  ...
  const hasActiveCycle = latestCycle && !['COMPLETED', 'CANCELLED'].includes(latestCycle.cycle_status)
  ...
  const getStatusInfo = (status: string) => {
    const statusMap: Record<string, { label: string; variant: 'default' | 'secondary' | 'destructive' | 'outline'; icon: React.ReactNode }> = {
      PENDING_PAYMENT: { label: 'Pending Payment', variant: 'secondary', icon: <Clock className="h-3 w-3" /> },
      PAYMENT_RECORDED: { label: 'Payment Recorded', variant: 'default', icon: <CreditCard className="h-3 w-3" /> },
      ...
      INVOICE_UPLOADED: { label: 'Invoice Uploaded', variant: 'default', icon: <FileText className="h-3 w-3" /> },
          ...
          {paymentCycles.length === 0 ? (
            ...
            <div className="space-y-4">
              {displayedCycles.map((cycle) => {
                ...
                const isOverdue = new Date(cycle.invoice_deadline) < new Date() && cycle.cycle_status !== 'COMPLETED' && cycle.cycle_status !== 'CANCELLED'
                ...
                return (
                  ...
                  >
                    ...
                    {/* Actions */}
                    {cycle.cycle_status !== 'COMPLETED' && cycle.cycle_status !== 'CANCELLED' && (
                      <div className="flex items-center gap-2 pt-2 border-t">
                        {(cycle.cycle_status === 'PENDING_PAYMENT' || cycle.cycle_status === 'PAYMENT_RECORDED') && (
                          <Button
                            variant="outline"
                            size="sm"
                          ...
                          >
                            ...
                            {cycle.cycle_status === 'PENDING_PAYMENT' ? 'Record Payment' : 'Update Payment'}
                          </Button>
                        )}
                        <Button
                          variant="ghost"
